...
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Technicians', href: '/admin/technician' },
];
...
export default function TechnicianIndex() {
    ...
    const handleApprove = (row: any) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.patch(route('admin.technician.approve', row.id), {}, { preserveScroll: true });
            }
        });
    };
    ...
    const handleReject = (row: any) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.patch(
                    route('admin.technician.reject', row.id),
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {items.map((row: any) => (
                    <PersonCard
                        ...
                        subtitle={
                            [
                                row.specialization ? row.specialization.replace(/_/g, ' ') : null,
                        ...
                        href={route('admin.technician.show', row.id)}
                        actions={

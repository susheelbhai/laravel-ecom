...
export default function PreviewBasicDetails({
    data,
    event_types,
    event_primary_foci,
}: any) {
    const getTitles = (ids: any[], list: any[]) =>
        Array.isArray(ids) && Array.isArray(list)
            ? list.filter((x) => ids.includes(x.id)).map((x) => x.title)
            : [];

    const festivalTypes = getTitles(data.event_type_ids, event_types);
    const primaryFocus = getTitles(
        data.event_primary_focus_ids,
        event_primary_foci,
    );
    ...
    return (
        <PreviewSection
            ...
            description="A quick summary of your festival’s identity and presentation."
        >
            ...
            <PreviewItem
                label="Festival Website / Social Media"
                value={data.website}
            />
            <PreviewImages label="Logo" urls={data.logo || data.media} single />
            <PreviewImages
                label="Cover Pic"
                urls={data.cover_pic || data.media}
                single
            />
            <PreviewImages
                label="Gallery Images"
                urls={data.gallery_images || data.media}
            />
        </PreviewSection>
    );
}

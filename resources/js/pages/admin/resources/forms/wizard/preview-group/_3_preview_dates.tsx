...
export default function PreviewImportantDates({ data }: { data: any }) {
    ...
    return (
        <>
            {/* === Festival Dates === */}
            <PreviewSection
                title="Festival Dates & Location"
                description="Timeline and important dates for submissions and events."
            >
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <PreviewItem
                        label="Opening Date"
                        value={formatDate(
                            data.opening_date || event?.opening_date,
                        )}
                    />
                    <PreviewItem
                        label="Notification Date"
                        value={formatDate(
                            data.notification_date || event?.notification_date,
                        )}
                    />
                    <PreviewItem
                        label="Event Start Date"
                        value={formatDate(data.start_date || event?.start_date)}
                    />
                    <PreviewItem
                        label="Event End Date"
                        value={formatDate(data.end_date || event?.end_date)}
                    />
                </div>
            ...
            <PreviewSection
                ...
                description="Competition categories with their respective entry fees and deadlines."
            >
                {categories.length > 0 ? (

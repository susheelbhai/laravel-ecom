...
export default function AdminDistributorStockShow() {
    const { owner, data } = usePage<SharedData>().props as any;
    const items = data?.data || [];

    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Stock', href: '/admin/stock/dashboard' },
        { title: 'Distributors', href: '/admin/stock/distributors' },
        { title: owner?.name ?? 'Distributor', href: '#' },
    ];
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="mb-4 text-sm text-gray-600">
                Distributor: {owner?.name} {owner?.email ? `(${owner.email})` : ''}
            </div>

    ...
    useEffect(() => {
        ...
        if (!editorUiColor) {
            try {
                ...
                const primaryColor = computedStyle
                    .getPropertyValue(CKEDITOR_STYLES.footerBgCssVar)
                    ?.trim();
                const backgroundColor = computedStyle
                    .getPropertyValue(CKEDITOR_STYLES.headerBgCssVar)
                    ?.trim();

                if (primaryColor) {
                    primaryColorValue = primaryColor;
                    // Convert CSS variable format to hex if needed
                    editorUiColor = primaryColor.startsWith('#')
                        ? primaryColor
                        : `hsl(${primaryColor})`;
                }
                ...
                console.warn(
                    'Could not read primary color from CSS variables:',
                    error,
                );
            }
        }
        ...
        if (primaryColorValue || uiColor) {
            ...
            if (!existingStyle) {
                ...
                const primaryToUse =
                    uiColor ||
                    (primaryColorValue.startsWith('#')
                        ? primaryColorValue
                        : `hsl(${primaryColorValue})`);
                const backgroundToUse = backgroundColorValue.startsWith('#')
                    ? backgroundColorValue
                    : `hsl(${backgroundColorValue})`;
        ...
        const loadEditor = async () => {
            ...
            if (!window.CKEDITOR) {
                ...
                script.src =
                    '/themes/ck_editor/vendor_components/ckeditor/ckeditor.js';
                script.onload = () => {
                    initEditor();
                };

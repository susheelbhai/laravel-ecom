...
export default function InputMultiImage({
    ...
    const value = Array.isArray(data?.[name]) && data[name] ? data[name] : [];
    ...
    return (
        <InputWrapper className={className}>
            ...
            <div
                ...
                onDrop={handleDrop}
                className={`relative flex cursor-pointer items-center justify-center overflow-hidden rounded-md border-2 border-dashed transition-colors ${dragging ? 'border-secondary bg-input-focused-bg' : 'border-input-border bg-input-bg hover:bg-input-hover-bg'}`}
            ...
            {previews.length > 0 && (
                <div
                    className="mt-2 grid gap-2 overflow-y-auto"
                    style={{
                ...
                >
                    {previews.map((url, idx) => (
                        <div
                            key={idx}
                            className="group relative overflow-hidden rounded-md border border-input-border"
                        ...
                        >
                            ...
                            <button
                                ...
                                type="button"
                                className="bg-opacity-60 absolute top-1 right-1 rounded bg-black px-2 py-0.5 text-xs text-white opacity-0 transition group-hover:opacity-100"
                            >
                                ✕
                            </button>
                        </div>

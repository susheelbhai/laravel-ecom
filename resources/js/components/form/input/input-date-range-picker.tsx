...
export default function InputDateRangePicker({
    ...
    const handleSelect = (selected: { from?: Date; to?: Date } | undefined) => {
        ...
        setData(
            name + '_from',
            safeRange.from ? formatLocalDate(safeRange.from) : '',
        );
        setData(
            name + '_to',
            safeRange.to ? formatLocalDate(safeRange.to) : '',
        );
        // Only close if both from and to are selected and they are not the same day
        if (
            safeRange.from &&
            safeRange.to &&
            safeRange.from.getTime() !== safeRange.to.getTime()
        ) {
            setIsOpen(false);
        }
    };
            ...
            <Popover open={isOpen} onOpenChange={setIsOpen}>
                <PopoverTrigger asChild>
                    <Button
                        ...
                        className={
                            className +
                            ' w-full justify-start border-input-border bg-input-bg text-left font-normal text-input-text hover:bg-input-hover-bg'
                        }
                        disabled={readOnly}
                    >

...
export default function InputMultiCheckbox({
    ...
    const handleToggle = (optionValue: string, checked: boolean) => {
        ...
        const numeric = updated.map((v) => (/^\d+$/.test(v) ? Number(v) : v));
    ...
    return (
        <InputWrapper className={className}>
            ...
            <div className="flex flex-col gap-2 rounded-md border-2 border-input-border bg-input-bg p-3 transition-colors focus-within:border-secondary/60 focus-within:bg-input-focused-bg">
                {(options ?? []).map((option: any) => {
                    ...
                    return (
                        <label
                            key={value}
                            className="flex cursor-pointer items-center gap-2 text-foreground select-none"
                        >
                            <input
                                type="checkbox"

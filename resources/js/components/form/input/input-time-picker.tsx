...
function CustomTimeDropdown({
    ...
    const isPeriodDropdown =
        options.length === 2 &&
        options[0].label === 'AM' &&
        options[1].label === 'PM';
    const infiniteOptions = isPeriodDropdown
        ? options
        : useMemo(() => {
    ...
    return (
        <div className="relative inline-block" ref={containerRef}>
            <button
                ...
                className="flex min-w-[40px] items-center justify-center gap-1 border-none bg-transparent p-0 text-sm font-medium transition-colors outline-none hover:text-primary"
            >
                {selectedOption?.label || value}
                <ChevronDown className="h-3 w-3" />
            </button>
            {isOpen && (
                <div className="absolute top-1/2 left-1/2 z-50 max-h-[200px] min-w-[80px] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-md border border-border bg-background py-3 shadow-lg">
                    <div
                        ref={dropdownRef}
                    ...
                    >
                        {infiniteOptions.map((option, index) => {
                            ...
                            if (isPeriodDropdown) {
                                ...
                                isSelected =
                                    option.value === value && isMiddleSet;
                            }
                            return (
                                <button
                                    ...
                                    className={cn(
                                        'time-dropdown-item w-full cursor-pointer border-none px-3 py-1.5 text-center text-sm transition-colors outline-none',
                                        isSelected &&
                ...
                <PopoverContent className="w-auto p-4" align="start">
                    ...
                    <div className="mt-3">
                        <Input
                            ...
                            className="border-input-border bg-input-bg text-center text-input-text placeholder:text-input-placeholder hover:bg-input-hover-bg focus:bg-input-focused-bg focus:text-input-focused-text"
                        />
                    </div>
                </PopoverContent>
            </Popover>

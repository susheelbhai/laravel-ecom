...
export default function Button({
    ...
    // If method is delete and style is default primary, use danger instead
    const buttonStyle =
        method === 'delete' && style === 'primary' ? 'danger' : style;

    // 1) if it's a post action, call the "other" component:
    if (method === 'post') {
        return (
            <ButtonComponent
                className={cn(StyleIt(buttonStyle), className)}
                size={size}
    ...
    if (!href) {
        ...
        return (
            <LinkComponent
                href={href}
                className={cn(StyleIt(buttonStyle), className, 'block')}
                size={size}
                {...props}
            >
                {text ?? children}
...
function ButtonComponent({
    ...
    return (
        <button
            className={cn(
                'cursor-pointer items-center rounded text-center',
                SizeIt(size),
                className,
            )}
            {...props}

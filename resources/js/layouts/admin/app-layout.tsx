...
export default ({ children, breadcrumbs, title, ...props }: AppLayoutProps) => {
    ...
    const admin = page.props.admin as
        | { unread_notifications_count?: number; unread_notifications?: any[] }
        | undefined;

    breadcrumbs = [
        { title: 'Dasshboard', href: '/admin' },
        ...(breadcrumbs || []),
    ];
    const unreadNotificationsCount = admin?.unread_notifications_count ?? 0;
    const unreadNotifications = admin?.unread_notifications ?? [];

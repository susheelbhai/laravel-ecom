...
export default ({ children, breadcrumbs, title, ...props }: AppLayoutProps) => {
    ...
    const seller = page.props.seller as
        | { unread_notifications_count?: number; unread_notifications?: any[] }
        | undefined;

    breadcrumbs = [
        { title: 'Dasshboard', href: '/seller' },
        ...(breadcrumbs || []),
    ];
    const unreadNotificationsCount = seller?.unread_notifications_count ?? 0;
    const unreadNotifications = seller?.unread_notifications ?? [];
